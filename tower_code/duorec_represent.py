...
# ================================================================
# [신규] PromptIntegratedTransformerLayer
# Pretrained Vector를 Key/Value 공간에 Prefix Prompt로 동적 주입
# ================================================================
class PromptIntegratedTransformerLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropout=0.1, attn_dropout=0.05):
        super().__init__()
        # Query, Key, Value를 다르게 받을 수 있도록 범용 MultiheadAttention 사용
    ...
    def forward(self, query_id, prompt_kv, src_mask=None, src_key_padding_mask=None):
        """
        query_id: (B, S, D) - 순수 ID 및 행동 맥락 스트림 (Query 전용)
        prompt_kv: (B, S, D) - SimCLR 사전 학습 메타데이터 스트림 (Prefix Prompt 용)
        """
        # [핵심 로직 1] Prefix Concat (시퀀스 차원으로 결합)
        # Key와 Value 공간에만 Prompt를 시퀀스 앞단에 이어 붙입니다. -> (B, 2S, D)
        kv_seq = torch.cat([prompt_kv, query_id], dim=1)
        
        # [핵심 로직 2] Causal Mask 확장
        # Query(S)가 Key(2S)를 바라볼 때의 마스킹 규칙 적용
        # src_mask shape: (S, S) -> 확장 후: (S, 2S)
        if src_mask is not None:
            # Query i는 Prompt 0~i 까지, 그리고 ID 0~i 까지 볼 수 있습니다.
            combined_mask = torch.cat([src_mask, src_mask], dim=1)
        else:
            combined_mask = None
            
        # [핵심 로직 3] Padding Mask 확장
        # src_key_padding_mask shape: (B, S) -> 확장 후: (B, 2S)
        if src_key_padding_mask is not None:
            combined_padding_mask = torch.cat([src_key_padding_mask, src_key_padding_mask], dim=1)
        else:
            combined_padding_mask = None

        # Attention 연산 (Pre-LayerNorm 구조 적용)
        normed_query = self.norm1(query_id)
        
        # Query는 S길이, Key/Value는 2S길이로 연산 수행
        attn_output, _ = self.multihead_attn(
            query=normed_query,
            key=kv_seq,
            value=kv_seq,
            attn_mask=combined_mask,
            key_padding_mask=combined_padding_mask,
            need_weights=False
        )
        
        # Residual Connection
        query_id = query_id + self.dropout1(attn_output)
        
        # FFN 연산
        normed_ffn = self.norm2(query_id)
        ff_output = self.linear2(self.dropout(self.activation(self.linear1(normed_ffn))))
        query_id = query_id + self.dropout2(ff_output)
        
        # 출력 차원은 다시 (B, S, D)로 유지됨
        return query_id


# ================================================================
# SASRecUserTower_v4 (Prompt-based 수정판 발췌)
# ================================================================
class SASRecUserTower_v4(nn.Module):
    def __init__(self, args):
        ...
        self.prompt_layers = nn.ModuleList([
            PromptIntegratedTransformerLayer(
                d_model=self.d_model,
                nhead=args.nhead,
                dim_feedforward=self.d_model * 2, # 일반적인 FFN 비율 적용
                dropout=0.15,
                attn_dropout=0.05,
            ) for _ in range(self.num_layers)
        ])
        
        self.final_enc_ln = nn.LayerNorm(self.d_model)
   ...
   # ── [2] Static Stream ────────────────────────────────────
        ...
        cont_out_dim = 64  # cont_mlp d_mode
    ...
    def forward(self,
        ...
        prompt_stream = self.emb_dropout(prompt_stream) # ✅ 추가
        ...
        id_stream    = self.emb_dropout(id_stream) # ✅ 추가
        
        # ====================================================================
        # Phase 3: Prompt-Integrated Encoding
        # ====================================================================
        hidden_id = id_stream
        
        for layer in self.prompt_layers:
            # hidden_id(Query)가 지속적으로 갱신되며 prompt_stream(사전)을 반복 조회
            hidden_id = layer(
                query_id=hidden_id,
                prompt_kv=prompt_stream,
                src_mask=causal_mask,
                src_key_padding_mask=padding_mask
            )
            
        item_output = self.final_enc_ln(hidden_id) # (B, S, D)
...
def log_feature_contributions_v4(model, wandb, epoch=None):
    ...
    with torch.no_grad():
        ...
        # ════════════════════════════════════════════════════════
        ln_norms = {
            'ItemStream/ln_prompt': model.ln_prompt.weight, 
            'ItemStream/ln_id':    model.ln_id.weight,
...
def train_user_tower_session_sampler_with_intent_point(
    ...
    lambda_unsup=0.1,       # Unsupervised CL 가중치
    lambda_sup=0.02,         # Supervised CL 가중치
    cl_temperature=0.1,
    ...
    lambdas = get_cl_lambdas(epoch, hnm_start_epoch=14)
    lambda_unsup = lambdas['lambda_unsup']
    lambda_sup   = lambdas['lambda_sup']
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        def _load_view(suffix=''):
            """suffix='' → view1, suffix='_v2' → view2"""
            s = suffix
            item_ids_ = batch[f'item_ids{s}'].to(device, non_blocking=True)
            if 'pretrained_vecs' in batch:
                pv = batch['pretrained_vecs'].to(device, non_blocking=True)
            else:
                pv = dataloader.dataset.pretrained_lookup[item_ids_.cpu()].to(device, non_blocking=True)
            return {
                'pretrained_vecs': pv,
                'item_ids':        item_ids_,
                'target_ids':      batch[f'target_ids{s}'].to(device, non_blocking=True),
                'padding_mask':    batch[f'padding_mask{s}'].to(device, non_blocking=True),
                'time_bucket_ids': batch[f'time_bucket_ids{s}'].to(device, non_blocking=True),
                'session_ids':     batch[f'session_ids{s}'].to(device, non_blocking=True),
                'type_ids':        batch[f'type_ids{s}'].to(device, non_blocking=True),
                'color_ids':       batch[f'color_ids{s}'].to(device, non_blocking=True),
                'graphic_ids':     batch[f'graphic_ids{s}'].to(device, non_blocking=True),
                'section_ids':     batch[f'section_ids{s}'].to(device, non_blocking=True),
                'price_bucket':    batch[f'price_bucket{s}'].to(device, non_blocking=True),
                'cnt_bucket':      batch[f'cnt_bucket{s}'].to(device, non_blocking=True),
                'recency_bucket':  batch[f'recency_bucket{s}'].to(device, non_blocking=True),
                'cont_feats':      batch[f'cont_feats{s}'].to(device, non_blocking=True),
                'channel_ids':     batch[f'channel_ids{s}'].to(device, non_blocking=True),
                'recency_offset':  batch[f'recency_offset{s}'].to(device, non_blocking=True),
                'current_week':    batch[f'current_week{s}'].to(device, non_blocking=True),
                'target_week':     batch[f'target_week{s}'].to(device, non_blocking=True),
                'interaction_dates': batch[f'interaction_dates{s}'].to(device, non_blocking=True),
                'age_bucket':      age_bucket,
                'club_status_ids': club_status_ids,
                'news_freq_ids':   news_freq_ids,
                'fn_ids':          fn_ids,
        ...
        def _make_forward_kwargs(v, training_mode=True):
            return {
                'pretrained_vecs': v['pretrained_vecs'],
                'item_ids':        v['item_ids'],
                'time_bucket_ids': v['time_bucket_ids'],
                'type_ids':        v['type_ids'],   'color_ids':    v['color_ids'],
                'graphic_ids':     v['graphic_ids'], 'section_ids': v['section_ids'],
                'age_bucket':      v['age_bucket'],  'price_bucket': v['price_bucket'],
                'cnt_bucket':      v['cnt_bucket'],  'recency_bucket': v['recency_bucket'],
                'channel_ids':     v['channel_ids'], 'club_status_ids': v['club_status_ids'],
                'news_freq_ids':   v['news_freq_ids'], 'fn_ids':    v['fn_ids'],
                'active_ids':      v['active_ids'],  'cont_feats': v['cont_feats'],
                'recency_offset':  v['recency_offset'], 'current_week': v['current_week'],
                'target_week':     v['target_week'], 'session_ids': v['session_ids'],
                'padding_mask':    v['padding_mask'],
                'training_mode':   training_mode
            }
 
        with torch.amp.autocast(device_type='cuda', dtype=torch.float16, enabled=True):
 
            # ── Forward: View 1 (Main Loss용) ────────────────────
            output_v1 = model(**_make_forward_kwargs(v1))
           # ── View 2 생성 (GPU 셔플) ────────────────────────────
            shuffle_idx = shuffle_within_session_gpu(
                v1['session_ids'], v1['padding_mask']
            )

            # item-level varying 텐서만 셔플
            # 정적 피처(age, club 등)는 세션과 무관 → 재사용
            v2 = {
                # 셔플 대상
                'pretrained_vecs': apply_shuffle(v1['pretrained_vecs'], shuffle_idx),
                'item_ids':        apply_shuffle(v1['item_ids'],        shuffle_idx),
                'time_bucket_ids': apply_shuffle(v1['time_bucket_ids'], shuffle_idx),
                'session_ids':     apply_shuffle(v1['session_ids'],     shuffle_idx),
                'type_ids':        apply_shuffle(v1['type_ids'],        shuffle_idx),
                'color_ids':       apply_shuffle(v1['color_ids'],       shuffle_idx),
                'graphic_ids':     apply_shuffle(v1['graphic_ids'],     shuffle_idx),
                'section_ids':     apply_shuffle(v1['section_ids'],     shuffle_idx),
                'cont_feats':      apply_shuffle(v1['cont_feats'],      shuffle_idx),
                'recency_offset':  apply_shuffle(v1['recency_offset'],  shuffle_idx),
                'current_week':    apply_shuffle(v1['current_week'],    shuffle_idx),
                'target_week':     apply_shuffle(v1['target_week'],     shuffle_idx),
                'price_bucket':    apply_shuffle(v1['price_bucket'],    shuffle_idx),
                'cnt_bucket':      apply_shuffle(v1['cnt_bucket'],      shuffle_idx),
                'recency_bucket':  apply_shuffle(v1['recency_bucket'],  shuffle_idx),
                'channel_ids':     apply_shuffle(v1['channel_ids'],     shuffle_idx),

                # 재사용 (셔플 불필요)
                'padding_mask':    v1['padding_mask'],   # ✅ padding 위치 유지됨
                'age_bucket':      v1['age_bucket'],
                'club_status_ids': v1['club_status_ids'],
                'news_freq_ids':   v1['news_freq_ids'],
                'fn_ids':          v1['fn_ids'],
                'active_ids':      v1['active_ids'],
            }

            # ── Forward: View 2 (Unsupervised CL용) ──────────────
            output_v2 = model(**_make_forward_kwargs(v2))
 
            batch_size, seq_len = v1['item_ids'].shape
            valid_mask_v1       = ~v1['padding_mask']
 
            # Time-Decay Weighting (v1 기준)
            interaction_dates = v1['interaction_dates']
            max_dates   = interaction_dates.masked_fill(v1['padding_mask'], -1).max(dim=1, keepdim=True)[0]
            delta_t     = (max_dates - interaction_dates).float()
            min_weight, half_life = 0.2, 21.0
            decay_rate  = math.log(2) / half_life
            seq_weights = min_weight + (1.0 - min_weight) * torch.exp(
                -decay_rate * delta_t
            )
 
            # session_last_mask (v1)
            session_ids_v1              = v1['session_ids']
            shifted_session_ids         = torch.roll(session_ids_v1, shifts=-1, dims=1)
            shifted_session_ids[:, -1]  = 0
            is_session_change           = (session_ids_v1 != shifted_session_ids)
            session_last_mask_v1        = is_session_change & valid_mask_v1
 
            # session_last_mask (v2)
            session_ids_v2              = v2['session_ids']
            valid_mask_v2               = ~v2['padding_mask']
            shifted_session_ids_v2      = torch.roll(session_ids_v2, shifts=-1, dims=1)
            shifted_session_ids_v2[:, -1] = 0
            is_session_change_v2        = (session_ids_v2 != shifted_session_ids_v2)
            session_last_mask_v2        = is_session_change_v2 & valid_mask_v2
 
            intermediate_mask      = valid_mask_v1 & ~session_last_mask_v1
            random_sample_mask     = torch.rand_like(intermediate_mask, dtype=torch.float) < 0.2
            final_loss_mask        = session_last_mask_v1 | (intermediate_mask & random_sample_mask)
 
            seq_positions = torch.arange(seq_len, device=device).unsqueeze(0).expand(batch_size, -1)
            batch_range   = torch.arange(batch_size, device=device)
            last_indices  = torch.max(
                seq_positions.masked_fill(~valid_mask_v1, -1), dim=1
            )[0].clamp(min=0)
            is_last_mask  = torch.zeros_like(valid_mask_v1, dtype=torch.bool)
            is_last_mask[batch_range, last_indices] = True
 
            # ── [1] Main Loss (v1 기반) ───────────────────────────
            flat_output   = output_v1[final_loss_mask]
            flat_targets  = v1['target_ids'][final_loss_mask]
            batch_row_idx = torch.arange(batch_size, device=device).unsqueeze(1).expand(-1, seq_len)
            ...
            if flat_output.size(0) > 0:
                ...
                flat_history_item_ids = v1['item_ids'][flat_user_ids]
            ...
            # ── [2] Unsupervised CL Loss ──────────────────────────
            # 동일 유저의 view1 / view2 세션 마지막 벡터를 positive pair로
            # v1, v2가 동일한 세션 구성이므로 세션 수와 순서가 같음
            last_v1 = output_v1[session_last_mask_v1]   # (N_sess, D)
            last_v2 = output_v2[session_last_mask_v2]   # (N_sess, D)
            cl_weights = seq_weights[session_last_mask_v1]
            # 세션 수가 다를 수 있으므로 최솟값 기준으로 맞춤
            min_sess = min(last_v1.size(0), last_v2.size(0))
            if min_sess > 1:
                loss_unsup = unsupervised_cl_loss(
                    last_v1[:min_sess],
                    last_v2[:min_sess],
                    temperature=cl_temperature,
                    weights=cl_weights[:min_sess] # 추가
                )
            else:
                loss_unsup = torch.tensor(0.0, device=device)
 
            # ── [3] Supervised CL Loss ────────────────────────────
            # 배치 내 same-user 세션 마지막 벡터를 positive pair로
            # flat_user_ids: final_loss_mask 기반 배치 행 인덱스
            sup_session_vecs    = output_v1[session_last_mask_v1]   # (N_sess, D)
            sup_user_row_ids    = batch_row_idx[session_last_mask_v1]  # (N_sess,)
            loss_sup, cl_metrics = supervised_cl_loss(
                session_vecs=sup_session_vecs,
                session_user_ids=sup_user_row_ids,
                temperature=cl_temperature,
                weights=cl_weights # 추가
            )
            b_metrics.update(cl_metrics)
 
            # ── [4] 최종 Loss ─────────────────────────────────────
            total_loss  = (
                main_loss
                + lambda_unsup * loss_unsup
                + lambda_sup   * loss_sup
            )
        ...
        unsup_cl_accum   += loss_unsup.item()
        sup_cl_accum     += loss_sup.item()
        num_batches      += 1
        ...
        if batch_idx % 100 == 0:
            wandb_log_dict = {
                ...
                "Train/Unsup_CL":    loss_unsup.item(),
                "Train/Sup_CL":      loss_sup.item(),
                "Train/Total_Loss":  total_loss.item(),
            }
 
            for k in [
                'sim/pos', 'sim/hn_max_mean', 'sim/hn_true_hard',
                'hn/discarded_ratio', 'hn/danger_zone_ratio',
                'loss/hn', 'loss/inbatch', 'loss/hn_ratio'
            ]:
                if k in b_metrics:
                    wandb_log_dict[k] = b_metrics[k]
 
            # CL 기여도
            denom = main_loss.item() + 1e-9
            wandb_log_dict['loss/unsup_cl_ratio'] = (lambda_unsup * loss_unsup).item() / denom
            wandb_log_dict['loss/sup_cl_ratio']   = (lambda_sup   * loss_sup).item()   / denom
 
            if 'cl/sup_has_peer_ratio' in b_metrics:
                wandb_log_dict['CL_Stats/Sup_Has_Peer_Ratio'] = b_metrics['cl/sup_has_peer_ratio']
                wandb_log_dict['CL_Stats/Sup_Avg_Peers']      = b_metrics['cl/sup_avg_peers']
 
            wandb.log(wandb_log_dict)
 
        # ── 메모리 정리 ───────────────────────────────────────────
        del output_v1, output_v2
        del flat_output, flat_targets, flat_user_ids, flat_weights
        del last_v1, last_v2, sup_session_vecs, sup_user_row_ids
        del main_loss, loss_unsup, loss_sup, scaled_loss, total_loss
    ...
    avg_unsup_cl = unsup_cl_accum   / len(dataloader)
    avg_sup_cl   = sup_cl_accum     / len(dataloader)
    ...
    print(f"🏁 Epoch {epoch} Completed | "
          f"Total: {avg_loss:.4f} | Main: {avg_main:.4f} | "
          f"Unsup_CL: {avg_unsup_cl:.4f} | Sup_CL: {avg_sup_cl:.4f}")
...
def train_pipeline_from_scratch():
    ...
    wandb.init(
        ...
        name=f"FromScratch_Duorec_lr{cfg.lr}_K{cfg.HN_K}", 
        config=cfg.__dict__ 
    )
    ...
    save_user_pth = os.path.join(cfg.model_dir, "best_user_tower_from_scratch_duorec.pth")
    save_item_pth = os.path.join(cfg.model_dir, "best_item_tower_from_scratch_duorec.pth")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        T_sample = max(0.45, 0.6 - int((epoch - 14) / 3) * 0.02)
        # ------------------- 훈련 (Train) -------------------
        ...
        if epoch == 13:
            ...
            base_user_pth =os.path.join(cfg.model_dir, "best_user_tower_from_scratch_base_duorec_d.pth")
            base_item_pth= os.path.join(cfg.model_dir, "best_item_tower_from_scratch_base_duorec_d.pth")
