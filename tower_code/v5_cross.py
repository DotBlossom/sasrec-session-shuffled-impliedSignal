...
def mine_global_hard_negatives_semantic_shield(
    user_embs,         # [N, D] 💡 유저의 현재 예측 벡터 (Anchor)
    target_ids,        # [N]    💡 유저의 실제 정답 아이템 ID (Target)
    item_embs,         # [Total_Items, D] CF 아이템 임베딩
    sbert_embs,        # [Total_Items, 384] S-BERT 임베딩
    fn_threshold=0.85, # 💡 이 유사도를 넘으면 "대체재(정답)"로 간주하고 오답 풀에서 삭제
    exclusion_top_k=20,# 💡 1위~20위는 여전히 가짜 오답일 확률이 높으니 배제 (Semi-hard)
    mine_k=50,         # 최종 추출할 HNM 개수
    device='cuda'
):
    """
    [Semantic-Denoised Semi-Hard Mining]
    CF 공간(user_embs)에서 헷갈리는 오답을 찾되, 
    S-BERT 공간에서 정답(target_ids)과 너무 비슷한 아이템은 '가짜 오답(FN)'으로 간주하고 필터링합니다.
    """
    N = user_embs.size(0)
    num_items = item_embs.size(0)
    print(f"🔍 Mining Global Hard Negatives using cached embeddings...")

    # 1. CF 공간에서의 유사도 계산 (모델이 착각하는 정도)
    # user_embs: [N, D], item_embs.T: [D, num_items]
    cf_sim_matrix = torch.matmul(user_embs, item_embs.T) # [N, num_items]
    
    # 2. S-BERT 공간에서의 유사도 계산 (실제 메타데이터의 유사성)
    # 정답 아이템의 S-BERT 벡터 추출
    target_sbert = sbert_embs[target_ids] # [N, 384]
    # 정답 S-BERT와 전체 아이템 S-BERT 비교
    semantic_sim_matrix = torch.matmul(target_sbert, sbert_embs.T) # [N, num_items]
    
    # 3. Semantic Shield (방어막) 가동
    # 정답과 속성이 너무 똑같으면(>0.85) 오답으로 쓰지 못하게 마스킹
    fn_mask = semantic_sim_matrix >= fn_threshold 
    
    # 자기 자신(정답) 및 패딩(0) 영구 배제
    batch_indices = torch.arange(N, device=device)
    cf_sim_matrix[batch_indices, target_ids] = -float('inf')
    cf_sim_matrix[:, 0] = -float('inf')
    
    # 가짜 오답(False Negative) 영구 배제
    cf_sim_matrix.masked_fill_(fn_mask, -float('inf'))
    
    # 통계 기록용
    total_masked = fn_mask.sum().item()
    
    # 4. Top-K Semi-Hard 추출
    total_k = min(exclusion_top_k + mine_k, num_items - 1)
    
    # CF 공간에서 가장 헷갈리는(점수가 높은) 놈들 추출
    _, topk_indices = torch.topk(cf_sim_matrix, total_k, dim=1)
    
    # 1등~20등(exclusion_top_k)은 너무 정답 같아서 버리고, 그 뒤부터 mine_k개 추출
    hard_neg_pool = topk_indices[:, exclusion_top_k : exclusion_top_k + mine_k]
    
    # 예외 처리: 패딩 채우기
    if hard_neg_pool.size(1) < mine_k:
        pad_size  = mine_k - hard_neg_pool.size(1)
        pad_idx   = hard_neg_pool[:, [0]].expand(-1, pad_size)
        hard_neg_pool = torch.cat([hard_neg_pool, pad_idx], dim=1)
        
    shield_metrics = {
        'HNM/semantic_shield_mask_ratio': total_masked / (N * num_items + 1e-9),
    }
...
class StreamFusionGate_p1(nn.Module):
    def __init__(self, d_model, reduction=4):
        ...
        self.register_buffer('_last_ratios', torch.zeros(3))
    def set_prior_bias(self, target_ratios=(0.85, 0.10, 0.05)):
        """
        💡 글로벌 초기화(apply) 이후에 호출되어 3-Way 스트림의 초기 주도권을 강제 세팅합니다.
        Softmax의 역함수(Log)를 이용하여 원하는 타겟 비율을 역산하여 Bias에 주입합니다.
        """
        final_linear = self.se[3]
        d = self.d_model
        
        with torch.no_grad():
            # 1. Weight를 완벽한 0으로 묶어 초기화 간섭 방지
            nn.init.constant_(final_linear.weight, 0.0)
            nn.init.constant_(final_linear.bias, 0.0) # 전체 0 초기화
            
            # 2. 양수 Logit 계산 및 주입 (85 : 10 : 5)
    ...
    def forward(self, h1, h2, h3):
        ...
        # 2. SE 네트워크 통과 (Logits 생성)
        gate_logits = self.se(combined)
        
        # 3. [핵심] 3개 스트림을 마주 보게 차원 재배열 (B, S, 3, D)
        gate_logits = gate_logits.view(h1.size(0), h1.size(1), 3, self.d_model)
        
        # 4. Softmax로 3자 경쟁 (합 = 1.0)
...
class DCNv2CrossLayer(nn.Module):
    def __init__(self, input_dim):
        super().__init__()
        # W * x + b 연산을 수행할 Linear 레이어
        self.cross_weight = nn.Linear(input_dim, input_dim)
        
        # 가중치 초기화 (Xavier Normal이 Cross Network에 유리)
        nn.init.xavier_normal_(self.cross_weight.weight)
        nn.init.constant_(self.cross_weight.bias, 0)

    def forward(self, x0, xl):
        # x0: 원본 피처 (B, S, D)
        # xl: 이전 레이어 통과 피처 (B, S, D)
        # DCN-V2 핵심 수식: x0 * (W * xl + b) + xl
        return x0 * self.cross_weight(xl) + xl

class FeatureCrossNetwork(nn.Module):
    def __init__(self, input_dim, num_layers=2):
        super().__init__()
        self.layers = nn.ModuleList([
            DCNv2CrossLayer(input_dim) for _ in range(num_layers)
        ])
...
class DecoupledTransformerLayer(nn.Module):
    ...
    def forward(self, id_stream, prompt_stream, src_mask=None, src_key_padding_mask=None):
        """
        [수정됨] Strictly Non-invasive (DIF-SR 스타일)
        ID와 Prompt는 어텐션 맵만 공유하고, 정보가 레이어 내부에서 절대 섞이지 않습니다.
        """
        ...
        # ================================================================
        # 5. [수정 포인트] 정보 주입 완전 철거 & 평행 업데이트
        # ================================================================
        # 💡 각자의 아웃풋을 각자의 스트림에만 더해줍니다. (섞임 X)
        id_stream     = id_stream + self.dropout1_id(self.out_proj_id(out_id))
        prompt_stream = prompt_stream + self.dropout1_prompt(self.out_proj_prompt(out_p))

        # 6. FFN & Residual Connection 2 (각자의 FFN을 통과)
...
class SASRecUserTower_v4(nn.Module):
    def __init__(self, args):
        ...
        self.final_enc_ln_prompt = nn.LayerNorm(self.d_model)
   ...
   # ── [2] Static Stream ────────────────────────────────────
        ...
        self.total_static_dim = mid_dim + (low_dim * 5) + num_asof
        
        # 1. 초경량 DCN-V2 Cross Network (단 2층으로 모든 피처의 상호작용 추출)
        self.cross_net = FeatureCrossNetwork(input_dim=self.total_static_dim, num_layers=2)
        
        # 2. 교차 완료된 40차원 벡터를 최종 d_model로 스케일업 & 정규화
        self.static_final_proj = nn.Sequential(
            nn.Linear(self.total_static_dim, self.d_model),
            nn.LayerNorm(self.d_model),
            nn.GELU(),
            nn.Dropout(self.dropout_rate) # 모래주머니
        )
        # ── [3] Fusion ───────────────────────────────────────────
    ...
    def forward(self,
        ...
        pos_embeddings = self.pos_emb(positions)  # (B, S, D)
        ...
        id_stream = self.id_final_ln(id_stream)             
        id_stream = self.emb_dropout(id_stream)
        
        # ====================================================================
        # Phase 3: Decoupled Prompt-Integrated Encoding (Strictly Non-invasive)
        # ====================================================================
        hidden_id = id_stream
        hidden_prompt = prompt_stream 
        
        for layer in self.decoupled_layers:
            # 💡 두 스트림은 내부에서 섞이지 않고 각자의 길을 갑니다.
        ...
        # 💡 ID 출력과 별도로, Prompt 출력도 정규화하여 준비해 둡니다.
        item_output   = self.final_enc_ln_id(hidden_id)       # (B, S, D)
        prompt_output = self.final_enc_ln_prompt(hidden_prompt) # (B, S, D) (init에 추가 필요: nn.LayerNorm(self.d_model))

        # ════════════════════════════════════════════════════════════
        # Phase 4: Static Stream (DCN-V2 Feature Crossing)
        # ════════════════════════════════════════════════════════════
        ...
        asof_feats = cont_feats[:, :, 1:]                                 

        static_concat = torch.cat([
            emb_age, emb_chan, emb_club, emb_news, emb_fn, emb_act, asof_feats
        ], dim=-1)
        
        crossed_feats = self.cross_net(static_concat)                     
        user_profile_vec = self.static_final_proj(crossed_feats)          

        # ════════════════════════════════════════════════════════════
        # Phase 5: target_week 벡터 (gate 유지)
        # ════════════════════════════════════════════════════════════
        ...
        # ════════════════════════════════════════════════════════════
        # Phase 6: 3-Way SENet Late Fusion (ID + Prompt + Static)
        # ════════════════════════════════════════════════════════════
        if training_mode:
            # 1. 3개의 스트림을 각각의 프로젝션 레이어로 통과 (체급 맞추기)
            h_seq_id = self.seq_proj(item_output)           # (B, S, D)
            h_seq_pr = self.prompt_proj(prompt_output)      # (B, S, D)
            h_static = self.static_proj(user_profile_vec)   # (B, S, D)

            # 2. 3-Way Softmax Fusion Gate 통과 (ID vs Prompt vs Static 경쟁)
            fused     = self.fusion_gate(h_seq_id, h_seq_pr, h_static)
            final_vec = self.output_proj(fused)

            # 3. 시간 정보(target_week) 주입 및 정규화
            if target_week is not None:
            ...
            # 💡 [중요] 추론(Evaluation) 시에는 마지막 타임스텝(-1)만 사용합니다.
            h_seq_id = self.seq_proj(item_output[:, -1:, :])          
            h_seq_pr = self.prompt_proj(prompt_output[:, -1:, :])     
            h_static = self.static_proj(user_profile_vec[:, -1:, :])  
...
def inbatch_corrected_logq_loss_unified(
    ...
    batch_hard_neg_ids=None, item_embedding_weight=None,
    step_weights=None, temperature=0.07, lambda_logq=1.0, margin=0.00,
    sbert_embs=None, semantic_threshold=0.88, # 💡 [추가] S-BERT 임베딩 및 대체재 판단 임계값
    ...
    if step_weights is not None:
        ...
        weight_sum = step_weights.sum() + 1e-8 
    else:
        weight_sum = None
    ...
    # 💡 핵심 1: 정답 유사도만 따로 빼냅니다. [N, 1]
    pos_sim = torch.diagonal(sim_matrix).unsqueeze(1)    
    
    # 💡 핵심 2: In-batch에서 정답(대각선)과 가짜 오답 마스킹
    ...
    # 🛡️ [신규 장착] In-batch Semantic Shield 가동!
    pure_semantic_masked_cnt = 0
    if sbert_embs is not None:
        with torch.no_grad():
            # 현재 배치 정답들의 S-BERT 벡터 추출 [N, 384]
            inbatch_sbert = sbert_embs[target_ids]
            # 인배치 아이템들끼리의 텍스트 유사도 계산 [N, N]
            inbatch_sem_sim = torch.matmul(inbatch_sbert, inbatch_sbert.T)
            # 임계값(0.88) 이상인 쌍둥이들을 찾아내어 True 마스킹
            semantic_twin_mask = inbatch_sem_sim >= semantic_threshold
            
            # 통계용: 본인(대각선)이나 동일 ID를 제외하고, '순수하게 텍스트만으로 걸러진 쌍'의 개수
            pure_semantic_masked_cnt = (semantic_twin_mask & ~diag_mask & ~same_item_mask).sum().item()
    else:
        semantic_twin_mask = torch.zeros((N, N), dtype=torch.bool, device=device)

    # 4종류의 마스크(대각선, 같은템, 같은유저, 시맨틱쌍둥이)를 모두 합쳐서 단번에 -inf 타격!
    total_inbatch_mask = same_item_mask | same_user_mask | diag_mask | semantic_twin_mask
    inbatch_sim = sim_matrix.masked_fill(total_inbatch_mask, SAFE_NEG_INF)

    # 2. Hard Negative 유사도 계산
    num_hn_to_use = 20
    if batch_hard_neg_ids is not None:
        rand_idx = torch.randperm(batch_hard_neg_ids.size(1), device=device)[:num_hn_to_use]
        batch_hn_ids_final = batch_hard_neg_ids[:, rand_idx] # [N, 20]
    ...
    # 3. 🚀 거대한 통합 Logit 생성: [정답, Inbatch 오답들, Hard 오답들] 순서로 이어 붙임
    all_sims = torch.cat([pos_sim, inbatch_sim, hn_sim], dim=1)
    logits = all_sims / temperature

    # (옵션) Margin 적용
    if margin > 0.0:
        logits[:, 0] -= (margin / temperature)

    # (옵션) 수학적으로 완벽한 LogQ 보정
    if lambda_logq > 0.0:
        pos_logq = log_q_tensor[target_ids].unsqueeze(1)                     # [N, 1]
        inbatch_logq = log_q_tensor[target_ids].unsqueeze(0).expand(N, N)    # [N, N]
        hn_logq = log_q_tensor[batch_hn_ids_final] if batch_hard_neg_ids is not None else torch.empty((N, 0), device=device)
        
        all_logq = torch.cat([pos_logq, inbatch_logq, hn_logq], dim=1)
        logits = logits - (all_logq * lambda_logq)

    # 4. 🚀 학습: 정답은 무조건 맨 앞(0번 인덱스)에 두었으므로 label은 All Zero!
    labels = torch.zeros(N, dtype=torch.long, device=device)

    if step_weights is not None:
        loss = F.cross_entropy(logits, labels, reduction='none')
        loss = (loss * step_weights).sum() / weight_sum
    else:
    ...
    if return_metrics:
        ...
        # 💡 [쉴드 통계 추가] 배치당 평균 몇 개의 가짜 오답이 죽었는지
        metrics['HNM/inbatch_shield_masked_cnt'] = pure_semantic_masked_cnt / N
        
        with torch.no_grad():
            if batch_hard_neg_ids is not None:
                metrics['sim/hn_max_mean'] = hn_sim.max(dim=1)[0].mean().item()
            
            # 💡 [핵심] Softmax 확률 분포 계산 (Gradient의 실제 영향력)
            probs = F.softmax(logits, dim=1)
            
            # 1. 정답 파이
            pos_prob_mass = probs[:, 0].mean().item()
            
            # 2. In-batch 오답 파이 (N개 합)
            inbatch_prob_mass = probs[:, 1 : 1 + N].sum(dim=1).mean().item()
            
            # 3. Hard Negative 파이 (20개 합)
            if batch_hard_neg_ids is not None:
                hn_prob_mass = probs[:, 1 + N :].sum(dim=1).mean().item()
            else:
                hn_prob_mass = 0.0
                
            metrics['prob_mass/pos'] = pos_prob_mass
            metrics['prob_mass/inbatch'] = inbatch_prob_mass
            metrics['prob_mass/hn'] = hn_prob_mass
...
def inbatch_corrected_logq_loss_with_hybrid_hard_neg(
    ...
    flat_history_item_ids=None, step_weights=None,
    temperature=0.05, lambda_logq=1.0, margin=0.00,
    T_HN=0.24, # 💡 [수정] 3-Way Gate의 충격을 줄이기 위해 조금 더 관대하게 시작
    beta=0.25, T_sample=0.5, return_metrics=False
):
    N = user_emb.size(0)
    device = user_emb.device
    SAFE_NEG_INF = -1e9
    ...
    # 2. Hard Negative Loss 계산 (이미 Semantic Shield를 거친 깨끗한 오답들)
    if batch_hard_neg_ids is not None:
        num_hn_to_use = 20 # 배치당 실제로 쓸 개수
        
        # 💡 HN 풀에서 임의 추출 (이미 앞단에서 필터링을 다 마쳤으므로 단순 추출만 함)
        rand_idx = torch.randperm(batch_hard_neg_ids.size(1), device=device)[:num_hn_to_use]
        batch_hn_ids_final = batch_hard_neg_ids[:, rand_idx] # [N, 20]
        ...
        if return_metrics:
            metrics['sim/hn_true_hard'] = hn_sim.mean().item()
            with torch.no_grad():
                max_hn_per_user, _ = hn_sim.max(dim=1)
                metrics['sim/hn_max_mean'] = max_hn_per_user.mean().item()
...
def log_feature_contributions_v4(model, wandb, epoch=None):
    """
    SASRecUserTower_v5 전용 정밀 로깅 함수
    수정 사항: 
      - DecoupledLayer 내부의 Dynamic Gate 로깅 완전 삭제 (Strictly Non-invasive 적용)
      - DCN-v2 (FeatureCrossNetwork) 내부 가중치 활성도 추적
      - 3-Way Softmax StreamFusionGate(ID_Seq vs Pr_Seq vs Static) 비중 추적
    """
    log_dict = {}
    ...
    with torch.no_grad():
        # ════════════════════════════════════════════════════════
        # 1. 스트림별 초기 진입로 에너지 (LayerNorm Scale)
        # ════════════════════════════════════════════════════════
        ln_modules = {
            'ItemStream/ln_prompt': model.ln_prompt,
            'ItemStream/ln_id':     model.ln_id,
            'ItemStream/ln_price':  model.ln_price,
            'ItemStream/final_ln_id':     model.final_enc_ln_id,
            'ItemStream/final_ln_prompt': model.final_enc_ln_prompt,
        }
        ...
        # ════════════════════════════════════════════════════════
        # 2. DCN-v2 Cross Network 분석 (피처 교차 강도)
        # ════════════════════════════════════════════════════════
        if hasattr(model, 'cross_net'):
            for i, layer in enumerate(model.cross_net.layers):
                # 가중치 놈이 높을수록 피처 간의 상호작용(교차)을 복잡하게 하고 있다는 뜻
                log_dict[f'DCN_Cross/layer_{i}_weight_norm'] = layer.cross_weight.weight.norm().item()
                log_dict[f'DCN_Cross/layer_{i}_bias_mean'] = layer.cross_weight.bias.mean().item()
        
        if hasattr(model, 'static_final_proj'):
        ...
        # ════════════════════════════════════════════════════════
        proj_modules = {
            'Proj_Norm/Seq_ID': model.seq_proj,
            'Proj_Norm/Seq_Prompt': model.prompt_proj,
            'Proj_Norm/Static': model.static_proj,
        }
        for name, proj in proj_modules.items():
            if hasattr(proj[0], 'weight'):
                log_dict[name] = proj[0].weight.norm().item()
...
def get_hnm_params(hnm_relative_epoch: int) -> dict:
    ...
    """
    if hnm_relative_epoch < 1:
        return {"T_HN": 0.0, "beta": 0.0}
    # 워밍업 스케줄 정의
    schedule = [
        # (T_HN, beta)
        (0.35, 0.05),   # epoch 1: 메인 온도의 4배 (매우 soft), HN 비중 최소
        (0.32, 0.10),   # epoch 2: 메인 온도의 3배
        (0.30, 0.15),   # epoch 3: 서서히 타겟에 근접
        (0.28, 0.20),   # epoch 4~: 목표값 도달 (메인 온도의 2배)
    ]
...
def get_top_k_params(curr_epoch: int) -> int:
    
    ex_top_k = 50
    
    if curr_epoch >= 6 and curr_epoch < 10:
        ex_top_k = 50
    
    if curr_epoch >= 10 and curr_epoch < 15:
        ex_top_k = 50 - (curr_epoch - 10 + 1) * 2
    
    if curr_epoch >= 16 and curr_epoch < 20:
        ex_top_k = 35 
...
def alignment_loss_fn(h_id, h_prompt):
    ...
    return (1.0 - cos).mean()
@torch.no_grad()
def build_item_level_hn_pool(item_embs, sbert_embs, fn_threshold=0.85, pool_size=400, batch_size=1024):
    """
    아이템 i와 CF적으로 유사하지만 S-BERT적으로는 다른 후보 400개를 미리 선정
    결과: [Total_Items, pool_size] 텐서
    """
    num_items = item_embs.size(0)
    device = item_embs.device
    global_pool = torch.zeros((num_items, pool_size), dtype=torch.long, device=device)
    
    for i in range(0, num_items, batch_size):
        end_i = min(i + batch_size, num_items)
        batch_items = item_embs[i:end_i]
        batch_sbert = sbert_embs[i:end_i]
        
        # 1. CF 유사도 & Semantic 유사도 (Batch x Total)
        cf_sim = torch.matmul(batch_items, item_embs.T)
        sem_sim = torch.matmul(batch_sbert, sbert_embs.T)
        
        # 2. Shield 적용 (S-BERT 유사도 높으면 제외)
        cf_sim.masked_fill_(sem_sim >= fn_threshold, -1e9)
        
        # 3. 자기 자신 제외
        diag_idx = torch.arange(i, end_i, device=device)
        cf_sim[torch.arange(end_i-i), diag_idx] = -1e9
        cf_sim[:, 0] = -1e9 # 패딩 제외
        
        # 4. Top-K 추출 (여기서 Top 20을 버리는 대신 400개를 넉넉히 가져감)
        _, topk_idx = torch.topk(cf_sim, pool_size, dim=1)
        global_pool[i:end_i] = topk_idx
        
    return global_pool

@torch.no_grad()
def build_semantic_shield_index(sbert_embs, fn_threshold=0.85, max_neighbors=100):
    """
    모든 아이템에 대해 S-BERT 유사도가 높은 '가짜 오답(FN)' 인덱스를 미리 추출
    결과: [Total_Items, max_neighbors] (패딩은 0)
    """
    num_items = sbert_embs.size(0)
    # 메모리 효율을 위해 상위 100개만 저장 (보통 패션에서 0.85 넘는게 100개 이상 나오기 힘듦)
    shield_index = torch.zeros((num_items, max_neighbors), dtype=torch.long, device=sbert_embs.device)
    
    print(f"🛡️ Building Semantic Shield Index (Threshold: {fn_threshold})...")
    for i in tqdm(range(0, num_items, 1024)):
        end_i = min(i + 1024, num_items)
        # (Batch x Total)
        sim = torch.matmul(sbert_embs[i:end_i], sbert_embs.T)
        
        # 스레숄드 체크 및 인덱스 추출
        for rel_idx, row in enumerate(sim):
            abs_idx = i + rel_idx
            # 스레숄드 넘는 인덱스 (자기 자신 제외)
            fn_ids = torch.where(row >= fn_threshold)[0]
            fn_ids = fn_ids[fn_ids != abs_idx] # 자기 자신 제외
            
            if len(fn_ids) > 0:
                keep = min(len(fn_ids), max_neighbors)
                shield_index[abs_idx, :keep] = fn_ids[:keep]
                
    return shield_index
def mine_global_hard_negatives_optimized(
    user_embs,          # [N, D] 유저 예측 벡터
    target_ids,         # [N] 정답 아이템 ID
    item_embs,          # [Total_Items, D] 전체 아이템 임베딩
    shield_index,       # [Total_Items, 100] 미리 만든 방어막 인덱스
    exclusion_top_k=20,
    mine_k=50,
    device='cuda'
):
    """
    [User-Anchored Optimized HNM]
    1. CF 전역 스캔으로 헷갈리는 오답 후보군 추출
    2. Shield Index를 이용해 S-BERT 기반 가짜 오답(FN) 고속 마스킹
    3. Shield Metrics(마스킹 비율) 통계 이식 완료
    """
    N = user_embs.size(0)
    num_items = item_embs.size(0)

    # 1. CF 전역 유사도 계산 (회원님 철학: User vs All Items)
    cf_sim_matrix = torch.matmul(user_embs, item_embs.T) # [N, num_items]
    
    # 2. Semantic Shield 정보 추출 [N, 64]
    # 현재 배치의 정답 아이템들과 S-BERT 유사도가 0.85 이상인 '쌍둥이' 명단
    bad_item_ids = shield_index[target_ids] 
    
    # 📊 [이식 포인트] Shield Metrics 계산
    # shield_index에서 0이 아닌 값(실제 쌍둥이 아이템)의 개수를 셉니다.
    # (0은 패딩이므로 제외해야 정확한 통계가 나옵니다)
    total_masked = (bad_item_ids > 0).sum().item()
    
    # 3. ⚡ 고속 마스킹 (scatter_ 사용)
    cf_sim_matrix.scatter_(1, bad_item_ids, -float('inf'))
    
    # 4. 자기 자신(정답) 및 패딩(0) 영구 배제
    batch_indices = torch.arange(N, device=device)
    cf_sim_matrix[batch_indices, target_ids] = -float('inf')
    cf_sim_matrix[:, 0] = -float('inf')
    
    # 5. Top-K Semi-Hard 추출
    total_k = min(exclusion_top_k + mine_k, num_items - 1)
    _, topk_indices = torch.topk(cf_sim_matrix, total_k, dim=1)
    ...
    if hard_neg_pool.size(1) < mine_k:
        pad_size = mine_k - hard_neg_pool.size(1)
        hard_neg_pool = torch.cat([hard_neg_pool, hard_neg_pool[:, [0]].expand(-1, pad_size)], dim=1)
...
def train_user_tower_session_sampler_with_intent_point(
    ...
    hn_refresh_interval, hn_exclusion_top_k, hn_mine_k, T_HN, sbert_embs, sbert_embs_raw
):
    """
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        user_outputs, _ = model(**forward_kwargs, return_streams=True)
        ...
        random_sample_mask = torch.rand_like(intermediate_mask, dtype=torch.float) < 0.15
        ...
        if flat_output.size(0) > 0:
            ...
            if epoch >=8: # 테스트 후 4~5로 조정 권장
                with torch.no_grad():
                    ...
                    batch_hard_neg_ids,shield_metrics= mine_global_hard_negatives_optimized(
                        user_embs=flat_user_emb,
                        target_ids=flat_targets,
                        item_embs=current_norm_item_embs,
                        shield_index=sbert_embs, # 👈 사전 계산된 인덱스 주입
                        exclusion_top_k=ex_top_k,
                        mine_k=50,
                        device=device
                    )
            else: 
            ...
            main_loss, b_metrics =inbatch_corrected_logq_loss_unified(
                user_emb=flat_user_emb, 
                seq_item_emb=batch_seq_item_emb,
                target_ids=flat_targets, 
                user_ids=flat_user_ids,
                log_q_tensor=log_q_tensor,
                batch_hard_neg_ids=batch_hard_neg_ids, # 👈 채굴된 오답 풀 주입
        ...
        if batch_idx % 100 == 0:
            ...
            for k in ['sim/pos', 'sim/hn_max_mean', 'loss/hn_ratio','loss/hn', 'loss/inbatch', 'hn/discarded_ratio','prob_mass/pos','prob_mass/inbatch','prob_mass/hn']:
                if k in b_metrics: wandb_log[k] = b_metrics[k]
            if 'HNM/semantic_shield_mask_ratio' in shield_metrics:
                wandb_log['HNM/semantic_shield_mask_ratio'] = shield_metrics['HNM/semantic_shield_mask_ratio']
            if 'HNM/inbatch_shield_masked_cnt' in b_metrics:
                wandb_log['HNM/inbatch_shield_masked_cnt'] = b_metrics['HNM/inbatch_shield_masked_cnt']
            wandb.log(wandb_log)
...
def train_pipeline_from_scratch():
    ...
    save_user_pth = os.path.join(cfg.model_dir, "best_user_tower_from_scratch_att_hn.pth")
    save_item_pth = os.path.join(cfg.model_dir, "best_item_tower_from_scratch_att_hn.pth")
    ...
    early_stopping = EarlyStopping(patience=12, mode='max')
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        avg_loss, force_mining_next_epoch, avg_discard_ratio = train_user_tower_session_sampler_with_intent_point(
            ...
            hn_mine_k=400,
            T_HN=T_HN_val,
            sbert_embs=semantic_shield_idx,
            sbert_embs_raw=aligned_sbert_embs
        )
        ...
        if early_stopping.is_best:
            ...
            # 💡 [추가] 최고 성능일 때의 Optimizer와 Scheduler 상태도 저장
            torch.save(optimizer.state_dict(), os.path.join(cfg.model_dir, "best_optimizer_from_scratch.pth"))
            torch.save(scheduler.state_dict(), os.path.join(cfg.model_dir, "best_scheduler_from_scratch.pth"))
            print(f"   💾 Best model weights & states saved to: {cfg.model_dir}")
        ...
        if epoch in [3,5,8]:
            ...
            # 💡 [추가] 특정 에포크의 Optimizer와 Scheduler 상태 저장
            torch.save(optimizer.state_dict(), os.path.join(cfg.model_dir, f"optimizer_ep{epoch}.pth"))
            torch.save(scheduler.state_dict(), os.path.join(cfg.model_dir, f"scheduler_ep{epoch}.pth"))
...
def train_pipeline_from_checkpoint():
    ...
    aligned_sbert_embs = get_or_build_aligned_sbert_embeddings(processor, cfg.base_dir, device)
    semantic_shield_idx = build_semantic_shield_index(aligned_sbert_embs, fn_threshold=0.85)
    train_loader = create_dataloaders(processor, cfg, "2020-09-16", aligned_vecs, is_train=True)
    val_loader = create_dataloaders(val_processor, cfg, "2020-09-22", aligned_vecs, is_train=False)
